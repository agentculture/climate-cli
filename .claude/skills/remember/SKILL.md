...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope climate-cli
...
scope:** the default personal scope is **private** (`--scope climate-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `climate-cli`). Private records are served only to a recall in the same
