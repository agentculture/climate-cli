...
description: >
  ...
  this agent's PERSONAL, PUBLIC scope (`--scope climate-cli --visibility
...
scope:** the default personal scope is **public** (`--scope climate-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `climate-cli`). Public records are visible to any scope's recall (no
