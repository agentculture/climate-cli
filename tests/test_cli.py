"""Smoke tests for the climate-cli CLI entry point and its verbs."""
...
from climate.explain import known_paths
...
def test_no_args_prints_help(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main([])
    assert rc == 0
    assert "usage: climate-cli" in capsys.readouterr().out
...
def test_whoami_text(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "nick: climate-cli" in out
...
def test_whoami_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload["nick"] == "climate-cli"
...
def test_learn_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["tool"] == "climate-cli"
    assert payload["version"] == __version__
...
def test_explain_root(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["explain"])
    assert rc == 0
    assert "# climate-cli" in capsys.readouterr().out


def test_explain_self(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["explain", "climate-cli"])
    assert rc == 0
    assert capsys.readouterr().out.startswith("#")


def test_explain_console_script_name(capsys: pytest.CaptureFixture[str]) -> None:
    # The console script is `climate` (pyproject [project.scripts]); the rubric
    # gate's explain_self check runs `explain climate`.
    rc = main(["explain", "climate"])
    assert rc == 0
    assert "# climate-cli" in capsys.readouterr().out
...
def test_explain_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "climate-cli whoami" in payload["markdown"]
