"""Unified CLI entry point for climate-cli.
...
--------------------------
Every handler raises :class:`climate.cli._errors.CliError` on
failure; ``main()`` catches it via :func:`_dispatch` and routes through
:mod:`climate.cli._output`. Unknown exceptions are wrapped into a
...
from climate.cli._errors import EXIT_USER_ERROR, CliError
from climate.cli._output import emit_error

_ISSUES_URL = "https://github.com/agentculture/climate-cli/issues"
