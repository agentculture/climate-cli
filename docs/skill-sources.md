...
climate-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. climate-cli
...
#   - consumer-identifying prose: `guildmaster` → `climate-cli` (NOT
...
If a re-sync would lose a climate-cli adaptation, lift the change
...
#     → `which climate-cli vendors from guildmaster`
