# Implementation Split Plan — weather tracking service

slug: `weather-tracking-service` · generated by `assign-to-workforce.sh split-plan --write` from `devague plan waves --json` (issue #82). Re-running this command overwrites this file in place; edits made to the **Task assignments** table below survive a regeneration, matched by task id.

## Dependency waves

- Wave 1: `t1`, `t2`, `t3`, `t4`, `t5`, `t6`, `t7`, `t8`
- Wave 2: `t9`, `t10`, `t11`, `t12`, `t13`, `t14`, `t15`, `t16`, `t17`, `t18`, `t19`
- Wave 3: `t20`, `t21`, `t22`, `t23`
- Wave 4: `t24`
- Wave 5: `t25`
- Wave 6: `t26`
...
| `t3` | subagent, isolated worktree | opus |
| `t4` | subagent, isolated worktree | sonnet |
| `t5` | subagent, isolated worktree | opus |
| `t6` | subagent, isolated worktree | opus |
| `t7` | subagent, isolated worktree | sonnet |
| `t8` | subagent, isolated worktree | sonnet |
| `t9` | subagent, isolated worktree | sonnet |
| `t10` | subagent, isolated worktree | sonnet |
| `t11` | subagent, isolated worktree | sonnet |
| `t12` | subagent, isolated worktree | sonnet |
| `t13` | subagent, isolated worktree | sonnet |
| `t14` | subagent, isolated worktree | sonnet |
| `t15` | subagent, isolated worktree | sonnet |
| `t16` | subagent, isolated worktree | sonnet |
| `t17` | subagent, isolated worktree | sonnet |
| `t18` | subagent, isolated worktree | sonnet |
| `t19` | subagent, isolated worktree | opus |
| `t20` | subagent, isolated worktree | sonnet |
| `t21` | subagent, isolated worktree | sonnet |
| `t22` | subagent, isolated worktree | opus |
| `t23` | subagent, isolated worktree | sonnet |
| `t24` | subagent, isolated worktree | sonnet |
| `t25` | subagent, isolated worktree | opus |
| `t26` | main agent (needs live docker + user go-ahead) | fable |
...
- One CLI stack command brings up weather-mongodb, the tracker and the web service; from then on every provider is sampled on its own refresh policy every 5 minutes, every response is kept in full, 'latest reading' answers in markdown or --json, and the dashboard charts what has been collected
...
- [unknown_nonblocking] The ims adapter is built from documentation only: no token exists yet (requested by email 2026-09-17), so payload shape, station ids near the configured location, cadence and rate limits are unverified until it arrives
- [unknown_nonblocking] The openweather fixture is synthesized from documented shapes because no API key was used during planning; the first live fetch may reveal normalization gaps (raw storage is unaffected)
- [unknown_nonblocking] The 24-hour 95% success figure and the reboot-resume check (c36) cannot be observed inside a build wave; t26 records them as evidence only when actually observed, otherwise they stay open
...
- [unknown_nonblocking] Approved obligation o8 is drifted and superseded by o15 (query verbs need no extra after decision c49); devague cannot reject an approved obligation, so tasks build to o15 and validate-delivery should file evidence against o15, not o8
